        ...
        $(document).ready(function () {
            ...
            function displayTrainingData(data) {
                ...
                for (var i = 0; i < trainings.length; i++) {
                    ...
                    row.insertCell(4).innerHTML = training.vcapacity;
                    row.insertCell(5).innerHTML = training.tdescription;
                    row.insertCell(6).innerHTML = training.apply_start_dt;
                    row.insertCell(7).innerHTML = training.apply_end_dt;
                    row.insertCell(8).innerHTML = training.training_start_dt;
                    row.insertCell(9).innerHTML = training.tstarttime;
                    row.insertCell(10).innerHTML = training.training_end_dt;
                    row.insertCell(11).innerHTML = training.tendtime;
                    row.insertCell(12).innerHTML = training.venue_id;
                    row.insertCell(13).innerHTML = training.venue_name + ', ' + training.vaddress;
                    row.insertCell(14).innerHTML = training.maplocation;
                    row.insertCell(15).innerHTML = training.coordinatorname;
                    row.insertCell(16).innerHTML = training.resourcetype;


                    var applyNowButton = document.createElement("button");
                    applyNowButton.textContent = "Apply Now";
                    applyNowButton.className = "xyz";
                    applyNowButton.dataset.venueCapacity = training.vcapacity;

                    // Use the createApplyButtonClickHandler to create the click handler
                    applyNowButton.onclick = createApplyButtonClickHandler(applyNowButton, training.ref_planner_id, training.venue_id, training.vcapacity);

                    var actionCell = row.insertCell(17);
                    actionCell.appendChild(applyNowButton);
                }
            }

            function createApplyButtonClickHandler(applyNowButton, refPlannerId, venueId, venueCapacity) {
                return function () {
                    checkVenueCapacityAndApply(applyNowButton, refPlannerId, venueId, venueCapacity);
                };
            }

            function checkVenueCapacityAndApply(applyNowButton, refPlannerId, venueId, venueCapacity) {
                $.ajax({
                    type: "GET",
                    url: "/api/countVenueOnTrainings",
                    data: {
                    ...
                    success: function (response) {
                        ...
                        if (response >= venueCapacity) {
                            applyNowButton.disabled = true;
                            applyNowButton.title = "Venue capacity already exceeds. Cannot apply.";
